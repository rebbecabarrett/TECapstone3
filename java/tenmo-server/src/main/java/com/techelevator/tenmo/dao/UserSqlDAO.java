...
public class UserSqlDAO implements UserDAO {
	...
	public List<Transfer> getListOfTransfers(int userId) {
		...
		SqlRowSet requestList = jdbcTemplate.queryForRowSet(
				"SELECT transfer_id, account_from, account_to, transfer_types.transfer_type_desc, transfer_statuses.transfer_status_desc, amount, users.username"
						...
						+ " WHERE users.user_id = ? AND transfer_types.transfer_type_desc = 'Request'", userId);

		while (requestList.next()) {

			request = mapRowToTransferWithName(requestList);
			listAllTransfers.add(request);
		}

		SqlRowSet sendList = jdbcTemplate.queryForRowSet(
				"SELECT transfer_id, account_from, account_to, transfer_types.transfer_type_desc, transfer_statuses.transfer_status_desc, amount, users.username"
						+ " FROM transfers" + " JOIN accounts ON accounts.account_id = transfers.account_to"
						+ " JOIN users ON users.user_id = accounts.user_id"
						+ " JOIN transfer_types ON transfer_types.transfer_type_id = transfers.transfer_type_id"
						+ " JOIN transfer_statuses ON transfer_statuses.transfer_status_id = transfers.transfer_status_id"
						+ " WHERE users.user_id = ? AND transfer_types.transfer_type_desc = 'Send'", userId);

		while (sendList.next()) {
			send = mapRowToTransferWithName(sendList);
			listAllTransfers.add(send);
		}
	...
	public String getUsernameFromAccountId(int accountId) {
		SqlRowSet response = jdbcTemplate.queryForRowSet(
				"select users.username JOIN accounts ON accounts.user_id = users.user_id where accounts.account_id = ?",
				accountId);
		if (response.next()) {
	...
	private Transfer mapRowToTransferWithName(SqlRowSet rs) {
		Transfer transfer = new Transfer();
		transfer.setTransferId(rs.getInt("transfer_id"));
		transfer.setAccount_from(rs.getInt("account_from"));
		transfer.setAccount_to(rs.getInt("account_to"));
